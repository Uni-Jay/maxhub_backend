...
const settingsService = new PHASE25_SettingsService();

export class PHASE25_SettingsController extends BaseController {
  /**
  ...
  async getDepartmentSettings(req: Request, res: Response): Promise<void> {
    try {
  ...
  async getEmailConfigs(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId } = req.params;
  ...
  async createEmailConfig(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId } = req.params;
      await this.checkPermission(req.user.id, 'settings.email.create.all');

      const config = await settingsService.createEmailConfig(
        Number(organizationId),
        req.body,
        req.user.id,
        req.ip || '',
  ...
  async updateEmailConfig(req: Request, res: Response): Promise<void> {
    try {
  ...
  async updateBrandingConfig(req: Request, res: Response): Promise<void> {
    try {
      ...
      const branding = await settingsService.updateBrandingConfig(
        Number(organizationId),
        req.body,
        req.user.id,
        req.ip || '',
  ...
  async getThemeConfigs(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId } = req.params;
  ...
  async createTheme(req: Request, res: Response): Promise<void> {
    try {
      ...
      const theme = await settingsService.createTheme(
        Number(organizationId),
        req.body,
        req.user.id,
        req.ip || '',
  ...
  async updateTheme(req: Request, res: Response): Promise<void> {
    try {
      ...
      const theme = await settingsService.updateTheme(
        Number(organizationId),
        Number(themeId),
        req.body,
        req.user.id,
  ...
  async getIntegrationConfigs(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId } = req.params;
  ...
  async createFeatureFlag(req: Request, res: Response): Promise<void> {
    try {
      ...
      const flag = await settingsService.createFeatureFlag(
        Number(organizationId),
        req.body,
        req.user.id
      );
  ...
  async updateFeatureFlag(req: Request, res: Response): Promise<void> {
    try {
      ...
      const flag = await settingsService.updateFeatureFlag(
        Number(organizationId),
        Number(flagId),
        req.body,
        req.user.id
  ...
  async getConfigVersions(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId, configType } = req.params;
      await this.checkPermission(req.user.id, 'settings.version.read.all');

      const versions = await settingsService.getConfigVersions(
        Number(organizationId),
        configType
      );
  ...
  async rollbackConfiguration(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await settingsService.rollbackConfiguration(
        Number(organizationId),
        configType,
        toVersionNumber,
        req.user.id
  ...
  async getSettingAuditLog(req: Request, res: Response): Promise<void> {
    try {
      const { organizationId } = req.params;
