/**
 * MaxHub ERP — Broadcast schema migration
 * One-off script: creates the broadcasts table (brand new, so plain
 * sequelize.sync() / CREATE TABLE IF NOT EXISTS is enough, no ALTER needed).
 *
 * Run: npx ts-node --transpile-only --require tsconfig-paths/register src/seeders/migrate-broadcasts.ts
 */
