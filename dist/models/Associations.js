"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AssociationManager = void 0;
const Conversation_model_1 = require("./Conversation.model");
const ConversationParticipant_model_1 = require("./ConversationParticipant.model");
const Message_model_1 = require("./Message.model");
const MessageRead_model_1 = require("./MessageRead.model");
const Branch_model_1 = require("./Branch.model");
const Unit_model_1 = require("./Unit.model");
const User_model_1 = require("./User.model");
const Role_model_1 = require("./Role.model");
const Permission_model_1 = require("./Permission.model");
const UserRole_model_1 = require("./UserRole.model");
const RolePermission_model_1 = require("./RolePermission.model");
const UserPermission_model_1 = require("./UserPermission.model");
const Session_model_1 = require("./Session.model");
const OTPVerification_model_1 = require("./OTPVerification.model");
const Department_model_1 = require("./Department.model");
const Designation_model_1 = require("./Designation.model");
const Location_model_1 = require("./Location.model");
const Staff_model_1 = require("./Staff.model");
const StaffQualification_model_1 = require("./StaffQualification.model");
const StaffSkill_model_1 = require("./StaffSkill.model");
const StaffDocument_model_1 = require("./StaffDocument.model");
const Shift_model_1 = require("./Shift.model");
const Attendance_model_1 = require("./Attendance.model");
const Overtime_model_1 = require("./Overtime.model");
const Timesheet_model_1 = require("./Timesheet.model");
const AttendanceLog_model_1 = require("./AttendanceLog.model");
const LeaveType_model_1 = require("./LeaveType.model");
const LeaveBalance_model_1 = require("./LeaveBalance.model");
const LeaveRequest_model_1 = require("./LeaveRequest.model");
const Project_model_1 = require("./Project.model");
const Milestone_model_1 = require("./Milestone.model");
const Task_model_1 = require("./Task.model");
const ProjectComment_model_1 = require("./ProjectComment.model");
const Contact_model_1 = require("./Contact.model");
const Opportunity_model_1 = require("./Opportunity.model");
const SalaryStructure_model_1 = require("./SalaryStructure.model");
const PayrollPeriod_model_1 = require("./PayrollPeriod.model");
const EmployeeSalary_model_1 = require("./EmployeeSalary.model");
const Course_model_1 = require("./Course.model");
const StaffDepartment_model_1 = require("./StaffDepartment.model");
const Meeting_model_1 = require("./Meeting.model");
const MeetingParticipant_model_1 = require("./MeetingParticipant.model");
const Call_model_1 = require("./Call.model");
const JobPosting_model_1 = require("./JobPosting.model");
const Company_model_1 = require("./Company.model");
const Program_model_1 = require("./Program.model");
const StudentProfile_model_1 = require("./StudentProfile.model");
const StudentEnrollment_model_1 = require("./StudentEnrollment.model");
const StudentResult_model_1 = require("./StudentResult.model");
const StudentAttendance_model_1 = require("./StudentAttendance.model");
const ClassSchedule_model_1 = require("./ClassSchedule.model");
const Module_model_1 = require("./Module.model");
const UserModulePermission_model_1 = require("./UserModulePermission.model");
const Enrollment_model_1 = require("./Enrollment.model");
const CourseModule_model_1 = require("./CourseModule.model");
const CourseContent_model_1 = require("./CourseContent.model");
const Exam_model_1 = require("./Exam.model");
const Question_model_1 = require("./Question.model");
const ExamResult_model_1 = require("./ExamResult.model");
const Certificate_model_1 = require("./Certificate.model");
const FeeReceipt_model_1 = require("./FeeReceipt.model");
const Appraisal_model_1 = require("./Appraisal.model");
const EmployeePromotion_model_1 = require("./EmployeePromotion.model");
const TrainingAttendance_model_1 = require("./TrainingAttendance.model");
const InventoryCategory_model_1 = require("./InventoryCategory.model");
const InventoryItem_model_1 = require("./InventoryItem.model");
const WarehouseStock_model_1 = require("./WarehouseStock.model");
const Supplier_model_1 = require("./Supplier.model");
const PurchaseOrder_model_1 = require("./PurchaseOrder.model");
const JobApplication_model_1 = require("./JobApplication.model");
const JobSyncLog_model_1 = require("./JobSyncLog.model");
const WeeklyReport_model_1 = require("./WeeklyReport.model");
const Customer_model_1 = require("./Customer.model");
const OrderTracking_model_1 = require("./OrderTracking.model");
const Invoice_model_1 = require("./Invoice.model");
const Client_model_1 = require("./Client.model");
const Quote_model_1 = require("./Quote.model");
const AIConversation_model_1 = require("../modules/ai/models/AIConversation.model");
const AIMessage_model_1 = require("../modules/ai/models/AIMessage.model");
const AIMeetingSummary_model_1 = require("../modules/ai/models/AIMeetingSummary.model");
const AIReminder_model_1 = require("../modules/ai/models/AIReminder.model");
class AssociationManager {
    static initializeAssociations(sequelize) {
        User_model_1.User.hasMany(Session_model_1.Session, { foreignKey: 'userId', as: 'sessions' });
        User_model_1.User.hasMany(OTPVerification_model_1.OTPVerification, { foreignKey: 'userId', as: 'otpVerifications' });
        User_model_1.User.belongsToMany(Role_model_1.Role, { through: UserRole_model_1.UserRole, foreignKey: 'userId', otherKey: 'roleId', as: 'roles' });
        User_model_1.User.belongsToMany(Permission_model_1.Permission, { through: UserPermission_model_1.UserPermission, foreignKey: 'userId', otherKey: 'permissionId', as: 'permissions' });
        User_model_1.User.hasMany(UserRole_model_1.UserRole, { foreignKey: 'userId', as: 'userRoles' });
        User_model_1.User.hasMany(UserPermission_model_1.UserPermission, { foreignKey: 'userId', as: 'userPermissions' });
        Role_model_1.Role.belongsToMany(Permission_model_1.Permission, { through: RolePermission_model_1.RolePermission, foreignKey: 'roleId', otherKey: 'permissionId', as: 'permissions' });
        Role_model_1.Role.hasMany(RolePermission_model_1.RolePermission, { foreignKey: 'roleId', as: 'rolePermissions' });
        Role_model_1.Role.hasMany(UserRole_model_1.UserRole, { foreignKey: 'roleId', as: 'userRoles' });
        Permission_model_1.Permission.hasMany(RolePermission_model_1.RolePermission, { foreignKey: 'permissionId', as: 'rolePermissions' });
        Permission_model_1.Permission.hasMany(UserPermission_model_1.UserPermission, { foreignKey: 'permissionId', as: 'userPermissions' });
        UserRole_model_1.UserRole.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        UserRole_model_1.UserRole.belongsTo(Role_model_1.Role, { foreignKey: 'roleId' });
        RolePermission_model_1.RolePermission.belongsTo(Role_model_1.Role, { foreignKey: 'roleId' });
        RolePermission_model_1.RolePermission.belongsTo(Permission_model_1.Permission, { foreignKey: 'permissionId' });
        UserPermission_model_1.UserPermission.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        UserPermission_model_1.UserPermission.belongsTo(Permission_model_1.Permission, { foreignKey: 'permissionId' });
        Session_model_1.Session.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        OTPVerification_model_1.OTPVerification.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        Branch_model_1.Branch.belongsTo(User_model_1.User, { foreignKey: 'managerId', as: 'manager' });
        Branch_model_1.Branch.hasMany(Department_model_1.Department, { foreignKey: 'branchId', as: 'departments' });
        Branch_model_1.Branch.hasMany(Unit_model_1.Unit, { foreignKey: 'branchId', as: 'units' });
        Branch_model_1.Branch.hasMany(Staff_model_1.Staff, { foreignKey: 'branchId', as: 'staff' });
        Unit_model_1.Unit.belongsTo(Branch_model_1.Branch, { foreignKey: 'branchId', as: 'branch' });
        Unit_model_1.Unit.belongsTo(User_model_1.User, { foreignKey: 'headUserId', as: 'head' });
        Unit_model_1.Unit.hasMany(Staff_model_1.Staff, { foreignKey: 'unitId', as: 'staff' });
        Staff_model_1.Staff.belongsTo(Branch_model_1.Branch, { foreignKey: 'branchId', as: 'branch' });
        Staff_model_1.Staff.belongsTo(Unit_model_1.Unit, { foreignKey: 'unitId', as: 'unit' });
        Department_model_1.Department.belongsTo(Branch_model_1.Branch, { foreignKey: 'branchId', as: 'branch' });
        Department_model_1.Department.belongsTo(Department_model_1.Department, { foreignKey: 'parentDepartmentId', as: 'parentDepartment' });
        Department_model_1.Department.hasMany(Department_model_1.Department, { foreignKey: 'parentDepartmentId', as: 'childDepartments' });
        Department_model_1.Department.belongsTo(User_model_1.User, { foreignKey: 'headUserId', as: 'head' });
        Department_model_1.Department.hasMany(Staff_model_1.Staff, { foreignKey: 'departmentId', as: 'staff' });
        Department_model_1.Department.hasMany(Designation_model_1.Designation, { foreignKey: 'departmentId', as: 'designations' });
        Designation_model_1.Designation.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId' });
        Designation_model_1.Designation.hasMany(Staff_model_1.Staff, { foreignKey: 'designationId', as: 'staff' });
        Location_model_1.Location.hasMany(Staff_model_1.Staff, { foreignKey: 'locationId', as: 'staff' });
        Staff_model_1.Staff.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        Staff_model_1.Staff.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        Staff_model_1.Staff.belongsToMany(Department_model_1.Department, { through: StaffDepartment_model_1.StaffDepartment, foreignKey: 'staffId', otherKey: 'departmentId', as: 'departments' });
        Staff_model_1.Staff.hasMany(StaffDepartment_model_1.StaffDepartment, { foreignKey: 'staffId', as: 'staffDepartments' });
        Department_model_1.Department.belongsToMany(Staff_model_1.Staff, { through: StaffDepartment_model_1.StaffDepartment, foreignKey: 'departmentId', otherKey: 'staffId', as: 'staffMembers' });
        Department_model_1.Department.hasMany(StaffDepartment_model_1.StaffDepartment, { foreignKey: 'departmentId', as: 'staffDepartments' });
        StaffDepartment_model_1.StaffDepartment.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        StaffDepartment_model_1.StaffDepartment.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId' });
        Staff_model_1.Staff.belongsTo(Designation_model_1.Designation, { foreignKey: 'designationId', as: 'designation' });
        Staff_model_1.Staff.belongsTo(Location_model_1.Location, { foreignKey: 'locationId' });
        Staff_model_1.Staff.belongsTo(Staff_model_1.Staff, { foreignKey: 'reportingManagerId', as: 'reportingManager' });
        Staff_model_1.Staff.hasMany(Staff_model_1.Staff, { foreignKey: 'reportingManagerId', as: 'subordinates' });
        Staff_model_1.Staff.hasMany(StaffQualification_model_1.StaffQualification, { foreignKey: 'staffId', as: 'qualifications' });
        Staff_model_1.Staff.hasMany(StaffSkill_model_1.StaffSkill, { foreignKey: 'staffId', as: 'staffSkills' });
        Staff_model_1.Staff.hasMany(StaffDocument_model_1.StaffDocument, { foreignKey: 'staffId', as: 'documents' });
        Staff_model_1.Staff.hasMany(Attendance_model_1.Attendance, { foreignKey: 'staffId', as: 'attendanceRecords' });
        Staff_model_1.Staff.hasMany(Timesheet_model_1.Timesheet, { foreignKey: 'staffId', as: 'timesheets' });
        Staff_model_1.Staff.hasMany(LeaveBalance_model_1.LeaveBalance, { foreignKey: 'staffId', as: 'leaveBalances' });
        Staff_model_1.Staff.hasMany(LeaveRequest_model_1.LeaveRequest, { foreignKey: 'staffId', as: 'leaveRequests' });
        Staff_model_1.Staff.hasMany(EmployeeSalary_model_1.EmployeeSalary, { foreignKey: 'staffId', as: 'salaries' });
        Staff_model_1.Staff.hasMany(Task_model_1.Task, { foreignKey: 'assigneeId', as: 'assignedTasks' });
        Staff_model_1.Staff.hasMany(Task_model_1.Task, { foreignKey: 'reporterId', as: 'reportedTasks' });
        Staff_model_1.Staff.hasMany(Project_model_1.Project, { foreignKey: 'projectManagerId', as: 'managedProjects' });
        Staff_model_1.Staff.hasMany(Course_model_1.Course, { foreignKey: 'instructorId', as: 'instructedCourses' });
        StaffQualification_model_1.StaffQualification.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        StaffSkill_model_1.StaffSkill.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        StaffDocument_model_1.StaffDocument.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        Shift_model_1.Shift.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId' });
        Shift_model_1.Shift.hasMany(Attendance_model_1.Attendance, { foreignKey: 'shiftId', as: 'attendanceRecords' });
        Attendance_model_1.Attendance.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        Attendance_model_1.Attendance.belongsTo(Shift_model_1.Shift, { foreignKey: 'shiftId' });
        Attendance_model_1.Attendance.belongsTo(User_model_1.User, { foreignKey: 'approvedBy', as: 'approver' });
        Overtime_model_1.Overtime.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        Overtime_model_1.Overtime.belongsTo(Attendance_model_1.Attendance, { foreignKey: 'attendanceId', as: 'attendance' });
        Attendance_model_1.Attendance.hasMany(AttendanceLog_model_1.AttendanceLog, { foreignKey: 'attendanceId', as: 'logs' });
        Timesheet_model_1.Timesheet.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        Timesheet_model_1.Timesheet.belongsTo(User_model_1.User, { foreignKey: 'approvedBy', as: 'approver' });
        AttendanceLog_model_1.AttendanceLog.belongsTo(Attendance_model_1.Attendance, { foreignKey: 'attendanceId' });
        AttendanceLog_model_1.AttendanceLog.belongsTo(User_model_1.User, { foreignKey: 'performedBy', as: 'performer' });
        LeaveType_model_1.LeaveType.hasMany(LeaveBalance_model_1.LeaveBalance, { foreignKey: 'leaveTypeId', as: 'balances' });
        LeaveType_model_1.LeaveType.hasMany(LeaveRequest_model_1.LeaveRequest, { foreignKey: 'leaveTypeId', as: 'requests' });
        LeaveBalance_model_1.LeaveBalance.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        LeaveBalance_model_1.LeaveBalance.belongsTo(LeaveType_model_1.LeaveType, { foreignKey: 'leaveTypeId', as: 'leaveType' });
        LeaveRequest_model_1.LeaveRequest.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        LeaveRequest_model_1.LeaveRequest.belongsTo(LeaveType_model_1.LeaveType, { foreignKey: 'leaveTypeId', as: 'leaveType' });
        LeaveRequest_model_1.LeaveRequest.belongsTo(User_model_1.User, { foreignKey: 'approverUserId', as: 'approver' });
        Project_model_1.Project.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId' });
        Project_model_1.Project.belongsTo(Staff_model_1.Staff, { foreignKey: 'projectManagerId', as: 'projectManager' });
        Project_model_1.Project.hasMany(Milestone_model_1.Milestone, { foreignKey: 'projectId', as: 'milestones' });
        Project_model_1.Project.hasMany(Task_model_1.Task, { foreignKey: 'projectId', as: 'tasks' });
        Project_model_1.Project.hasMany(Timesheet_model_1.Timesheet, { foreignKey: 'projectId', as: 'timesheets' });
        Milestone_model_1.Milestone.belongsTo(Project_model_1.Project, { foreignKey: 'projectId' });
        Milestone_model_1.Milestone.hasMany(Task_model_1.Task, { foreignKey: 'milestoneId', as: 'tasks' });
        Task_model_1.Task.belongsTo(Project_model_1.Project, { foreignKey: 'projectId' });
        Task_model_1.Task.belongsTo(Staff_model_1.Staff, { foreignKey: 'assigneeId', as: 'assignee' });
        Task_model_1.Task.belongsTo(Staff_model_1.Staff, { foreignKey: 'reporterId', as: 'reporter' });
        Task_model_1.Task.belongsTo(Task_model_1.Task, { foreignKey: 'parentTaskId', as: 'parentTask' });
        Task_model_1.Task.hasMany(Task_model_1.Task, { foreignKey: 'parentTaskId', as: 'subtasks' });
        Task_model_1.Task.belongsTo(Milestone_model_1.Milestone, { foreignKey: 'milestoneId' });
        Task_model_1.Task.hasMany(ProjectComment_model_1.ProjectComment, { foreignKey: 'taskId', as: 'comments' });
        ProjectComment_model_1.ProjectComment.belongsTo(Task_model_1.Task, { foreignKey: 'taskId' });
        ProjectComment_model_1.ProjectComment.belongsTo(Project_model_1.Project, { foreignKey: 'projectId' });
        ProjectComment_model_1.ProjectComment.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'author' });
        Project_model_1.Project.hasMany(ProjectComment_model_1.ProjectComment, { foreignKey: 'projectId', as: 'comments' });
        Contact_model_1.Contact.belongsTo(User_model_1.User, { foreignKey: 'ownerUserId', as: 'owner' });
        Contact_model_1.Contact.hasMany(Opportunity_model_1.Opportunity, { foreignKey: 'primaryContactId', as: 'opportunities' });
        Opportunity_model_1.Opportunity.belongsTo(Contact_model_1.Contact, { foreignKey: 'primaryContactId', as: 'primaryContact' });
        Opportunity_model_1.Opportunity.belongsTo(User_model_1.User, { foreignKey: 'ownerUserId', as: 'owner' });
        SalaryStructure_model_1.SalaryStructure.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId' });
        SalaryStructure_model_1.SalaryStructure.belongsTo(Designation_model_1.Designation, { foreignKey: 'designationId' });
        PayrollPeriod_model_1.PayrollPeriod.belongsTo(User_model_1.User, { foreignKey: 'processedBy', as: 'processedByUser' });
        PayrollPeriod_model_1.PayrollPeriod.belongsTo(User_model_1.User, { foreignKey: 'approvedBy', as: 'approvedByUser' });
        PayrollPeriod_model_1.PayrollPeriod.hasMany(EmployeeSalary_model_1.EmployeeSalary, { foreignKey: 'payrollPeriodId', as: 'employeeSalaries' });
        EmployeeSalary_model_1.EmployeeSalary.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId' });
        EmployeeSalary_model_1.EmployeeSalary.belongsTo(PayrollPeriod_model_1.PayrollPeriod, { foreignKey: 'payrollPeriodId' });
        Course_model_1.Course.belongsTo(Staff_model_1.Staff, { foreignKey: 'instructorId', as: 'instructor' });
        Course_model_1.Course.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        Course_model_1.Course.belongsTo(User_model_1.User, { foreignKey: 'createdById', as: 'createdBy' });
        Course_model_1.Course.hasMany(CourseModule_model_1.CourseModule, { foreignKey: 'courseId', as: 'modules' });
        Course_model_1.Course.hasMany(Exam_model_1.Exam, { foreignKey: 'courseId', as: 'exams' });
        CourseModule_model_1.CourseModule.hasMany(CourseContent_model_1.CourseContent, { foreignKey: 'moduleId', as: 'contents' });
        Exam_model_1.Exam.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        Exam_model_1.Exam.hasMany(Question_model_1.Question, { foreignKey: 'examId', as: 'questions' });
        Enrollment_model_1.Enrollment.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        Enrollment_model_1.Enrollment.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        ExamResult_model_1.ExamResult.belongsTo(Enrollment_model_1.Enrollment, { foreignKey: 'enrollmentId', as: 'enrollment' });
        Certificate_model_1.Certificate.belongsTo(Enrollment_model_1.Enrollment, { foreignKey: 'enrollmentId', as: 'enrollment' });
        FeeReceipt_model_1.FeeReceipt.belongsTo(Enrollment_model_1.Enrollment, { foreignKey: 'enrollmentId', as: 'enrollment' });
        FeeReceipt_model_1.FeeReceipt.belongsTo(User_model_1.User, { foreignKey: 'issuedById', as: 'issuedBy' });
        Invoice_model_1.Invoice.belongsTo(Client_model_1.Client, { foreignKey: 'clientId', as: 'client' });
        Invoice_model_1.Invoice.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        Invoice_model_1.Invoice.belongsTo(User_model_1.User, { foreignKey: 'createdById', as: 'createdBy' });
        Quote_model_1.Quote.belongsTo(Client_model_1.Client, { foreignKey: 'clientId', as: 'client' });
        Quote_model_1.Quote.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        Quote_model_1.Quote.belongsTo(Opportunity_model_1.Opportunity, { foreignKey: 'opportunityId', as: 'opportunity' });
        Quote_model_1.Quote.belongsTo(User_model_1.User, { foreignKey: 'createdById', as: 'createdBy' });
        JobPosting_model_1.JobPosting.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        JobPosting_model_1.JobPosting.belongsTo(Designation_model_1.Designation, { foreignKey: 'designationId', as: 'designation' });
        JobPosting_model_1.JobPosting.belongsTo(User_model_1.User, { foreignKey: 'createdById', as: 'createdBy' });
        JobApplication_model_1.JobApplication.belongsTo(JobPosting_model_1.JobPosting, { foreignKey: 'jobPostingId', as: 'jobPosting' });
        JobPosting_model_1.JobPosting.hasMany(JobSyncLog_model_1.JobSyncLog, { foreignKey: 'jobPostingId', as: 'syncLogs' });
        JobSyncLog_model_1.JobSyncLog.belongsTo(JobPosting_model_1.JobPosting, { foreignKey: 'jobPostingId', as: 'jobPosting' });
        WeeklyReport_model_1.WeeklyReport.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        Staff_model_1.Staff.hasMany(WeeklyReport_model_1.WeeklyReport, { foreignKey: 'staffId', as: 'weeklyReports' });
        WeeklyReport_model_1.WeeklyReport.belongsTo(User_model_1.User, { foreignKey: 'approvedById', as: 'approvedBy' });
        OrderTracking_model_1.OrderTracking.belongsTo(Customer_model_1.Customer, { foreignKey: 'customerId', as: 'customer' });
        Customer_model_1.Customer.hasMany(OrderTracking_model_1.OrderTracking, { foreignKey: 'customerId', as: 'orders' });
        AIConversation_model_1.AIConversation.belongsTo(User_model_1.User, { foreignKey: 'userId', as: 'user' });
        AIConversation_model_1.AIConversation.hasMany(AIMessage_model_1.AIMessage, { foreignKey: 'conversationId', as: 'messages' });
        AIMessage_model_1.AIMessage.belongsTo(AIConversation_model_1.AIConversation, { foreignKey: 'conversationId', as: 'conversation' });
        AIMeetingSummary_model_1.AIMeetingSummary.belongsTo(User_model_1.User, { foreignKey: 'userId', as: 'user' });
        AIReminder_model_1.AIReminder.belongsTo(User_model_1.User, { foreignKey: 'userId', as: 'user' });
        Appraisal_model_1.Appraisal.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        TrainingAttendance_model_1.TrainingAttendance.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        EmployeePromotion_model_1.EmployeePromotion.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        EmployeePromotion_model_1.EmployeePromotion.belongsTo(Designation_model_1.Designation, { foreignKey: 'fromDesignationId', as: 'fromDesignation' });
        EmployeePromotion_model_1.EmployeePromotion.belongsTo(Designation_model_1.Designation, { foreignKey: 'toDesignationId', as: 'toDesignation' });
        EmployeePromotion_model_1.EmployeePromotion.belongsTo(Department_model_1.Department, { foreignKey: 'fromDepartmentId', as: 'fromDepartment' });
        EmployeePromotion_model_1.EmployeePromotion.belongsTo(Department_model_1.Department, { foreignKey: 'toDepartmentId', as: 'toDepartment' });
        EmployeeSalary_model_1.EmployeeSalary.belongsTo(Staff_model_1.Staff, { foreignKey: 'staffId', as: 'staff' });
        EmployeeSalary_model_1.EmployeeSalary.belongsTo(PayrollPeriod_model_1.PayrollPeriod, { foreignKey: 'payrollPeriodId', as: 'payrollPeriod' });
        InventoryItem_model_1.InventoryItem.belongsTo(InventoryCategory_model_1.InventoryCategory, { foreignKey: 'categoryId', as: 'category' });
        WarehouseStock_model_1.WarehouseStock.belongsTo(InventoryItem_model_1.InventoryItem, { foreignKey: 'itemId', as: 'item' });
        PurchaseOrder_model_1.PurchaseOrder.belongsTo(Supplier_model_1.Supplier, { foreignKey: 'supplierId', as: 'supplier' });
        Company_model_1.Company.hasMany(Program_model_1.Program, { foreignKey: 'companyId', as: 'programs' });
        Company_model_1.Company.hasMany(StudentProfile_model_1.StudentProfile, { foreignKey: 'companyId', as: 'students' });
        Program_model_1.Program.belongsTo(Company_model_1.Company, { foreignKey: 'companyId', as: 'company' });
        Program_model_1.Program.hasMany(StudentProfile_model_1.StudentProfile, { foreignKey: 'programId', as: 'students' });
        User_model_1.User.hasOne(StudentProfile_model_1.StudentProfile, { foreignKey: 'userId', as: 'studentProfile' });
        StudentProfile_model_1.StudentProfile.belongsTo(User_model_1.User, { foreignKey: 'userId', as: 'user' });
        StudentProfile_model_1.StudentProfile.belongsTo(Company_model_1.Company, { foreignKey: 'companyId', as: 'company' });
        StudentProfile_model_1.StudentProfile.belongsTo(Program_model_1.Program, { foreignKey: 'programId', as: 'program' });
        StudentProfile_model_1.StudentProfile.belongsTo(Department_model_1.Department, { foreignKey: 'departmentId', as: 'department' });
        StudentProfile_model_1.StudentProfile.hasMany(StudentEnrollment_model_1.StudentEnrollment, { foreignKey: 'studentId', as: 'enrollments' });
        StudentEnrollment_model_1.StudentEnrollment.belongsTo(StudentProfile_model_1.StudentProfile, { foreignKey: 'studentId', as: 'student' });
        StudentEnrollment_model_1.StudentEnrollment.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        StudentEnrollment_model_1.StudentEnrollment.belongsTo(User_model_1.User, { foreignKey: 'enrolledById', as: 'enrolledBy' });
        Course_model_1.Course.hasMany(StudentEnrollment_model_1.StudentEnrollment, { foreignKey: 'courseId', as: 'studentEnrollments' });
        StudentProfile_model_1.StudentProfile.hasMany(StudentResult_model_1.StudentResult, { foreignKey: 'studentId', as: 'results' });
        StudentResult_model_1.StudentResult.belongsTo(StudentProfile_model_1.StudentProfile, { foreignKey: 'studentId', as: 'student' });
        StudentResult_model_1.StudentResult.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        StudentResult_model_1.StudentResult.belongsTo(User_model_1.User, { foreignKey: 'gradedById', as: 'gradedBy' });
        Course_model_1.Course.hasMany(StudentResult_model_1.StudentResult, { foreignKey: 'courseId', as: 'studentResults' });
        StudentProfile_model_1.StudentProfile.hasMany(StudentAttendance_model_1.StudentAttendance, { foreignKey: 'studentId', as: 'attendanceRecords' });
        StudentAttendance_model_1.StudentAttendance.belongsTo(StudentProfile_model_1.StudentProfile, { foreignKey: 'studentId', as: 'student' });
        StudentAttendance_model_1.StudentAttendance.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        StudentAttendance_model_1.StudentAttendance.belongsTo(User_model_1.User, { foreignKey: 'markedById', as: 'markedBy' });
        Course_model_1.Course.hasMany(StudentAttendance_model_1.StudentAttendance, { foreignKey: 'courseId', as: 'studentAttendance' });
        ClassSchedule_model_1.ClassSchedule.belongsTo(Course_model_1.Course, { foreignKey: 'courseId', as: 'course' });
        ClassSchedule_model_1.ClassSchedule.belongsTo(Staff_model_1.Staff, { foreignKey: 'instructorStaffId', as: 'instructor' });
        Course_model_1.Course.hasMany(ClassSchedule_model_1.ClassSchedule, { foreignKey: 'courseId', as: 'schedules' });
        Meeting_model_1.Meeting.belongsTo(User_model_1.User, { foreignKey: 'hostUserId', as: 'host' });
        User_model_1.User.hasMany(Meeting_model_1.Meeting, { foreignKey: 'hostUserId', as: 'hostedMeetings' });
        Meeting_model_1.Meeting.hasMany(MeetingParticipant_model_1.MeetingParticipant, { foreignKey: 'meetingId', as: 'participants' });
        MeetingParticipant_model_1.MeetingParticipant.belongsTo(Meeting_model_1.Meeting, { foreignKey: 'meetingId' });
        MeetingParticipant_model_1.MeetingParticipant.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        User_model_1.User.hasMany(MeetingParticipant_model_1.MeetingParticipant, { foreignKey: 'userId', as: 'meetingParticipations' });
        Call_model_1.Call.belongsTo(User_model_1.User, { foreignKey: 'callerUserId', as: 'caller' });
        Call_model_1.Call.belongsTo(User_model_1.User, { foreignKey: 'calleeUserId', as: 'callee' });
        User_model_1.User.hasMany(Call_model_1.Call, { foreignKey: 'callerUserId', as: 'outgoingCalls' });
        User_model_1.User.hasMany(Call_model_1.Call, { foreignKey: 'calleeUserId', as: 'incomingCalls' });
        Conversation_model_1.Conversation.hasMany(Message_model_1.Message, { foreignKey: 'conversationId', as: 'messages' });
        Message_model_1.Message.belongsTo(Conversation_model_1.Conversation, { foreignKey: 'conversationId', as: 'conversation' });
        Message_model_1.Message.belongsTo(User_model_1.User, { foreignKey: 'senderUserId', as: 'sender' });
        User_model_1.User.hasMany(Message_model_1.Message, { foreignKey: 'senderUserId', as: 'sentMessages' });
        Message_model_1.Message.belongsTo(Message_model_1.Message, { foreignKey: 'replyToMessageId', as: 'replyTo' });
        Message_model_1.Message.hasMany(MessageRead_model_1.MessageRead, { foreignKey: 'messageId', as: 'reads' });
        MessageRead_model_1.MessageRead.belongsTo(Message_model_1.Message, { foreignKey: 'messageId' });
        MessageRead_model_1.MessageRead.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        Conversation_model_1.Conversation.hasMany(ConversationParticipant_model_1.ConversationParticipant, { foreignKey: 'conversationId', as: 'participants' });
        ConversationParticipant_model_1.ConversationParticipant.belongsTo(Conversation_model_1.Conversation, { foreignKey: 'conversationId' });
        ConversationParticipant_model_1.ConversationParticipant.belongsTo(User_model_1.User, { foreignKey: 'userId', as: 'user' });
        User_model_1.User.hasMany(ConversationParticipant_model_1.ConversationParticipant, { foreignKey: 'userId', as: 'conversationParticipations' });
        User_model_1.User.hasMany(UserModulePermission_model_1.UserModulePermission, { foreignKey: 'userId', as: 'modulePermissions' });
        UserModulePermission_model_1.UserModulePermission.belongsTo(User_model_1.User, { foreignKey: 'userId' });
        Module_model_1.AppModule.hasMany(UserModulePermission_model_1.UserModulePermission, { foreignKey: 'moduleCode', sourceKey: 'code', as: 'userOverrides' });
        UserModulePermission_model_1.UserModulePermission.belongsTo(Module_model_1.AppModule, { foreignKey: 'moduleCode', targetKey: 'code', as: 'module' });
    }
}
exports.AssociationManager = AssociationManager;
//# sourceMappingURL=Associations.js.map